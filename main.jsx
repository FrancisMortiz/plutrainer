...
export default function KassentrainerApp() {
      ...
      <Rahmen>
        ...
        <div className="space-y-5">
          <div>
            ...
            <select value={kategorie} onChange={(e) => setKategorie(e.target.value)} className={`w-full border rounded-xl p-3 text-base ${farben.feld}`}>
              {kategorien.map((kat) => (
        ...
        <button onClick={() => quizStarten(false)} className="w-full mt-8 bg-green-600 text-white rounded-xl p-3 text-base font-semibold active:scale-[0.98] transition">
          Quiz starten
        </button>
  ...
  if (ansicht === "quiz" && aktuelleFrage) {
    return (
      <Rahmen>
        ...
        <div className={`w-full border rounded-2xl p-5 text-center text-3xl font-bold tracking-widest min-h-[64px] flex items-center justify-center ${farben.feld}`}>
          ...
          <div className="mt-3 grid grid-cols-3 gap-2">
            {[7, 8, 9, 4, 5, 6, 1, 2, 3].map((zahl) => (
              <button
                ...
                className={`rounded-xl p-3 text-xl font-bold active:scale-[0.96] transition ${farben.buttonSek}`}
              >
                {zahl}
              </button>
            ))}
            ...
            <button
              ...
              className={`rounded-xl p-3 text-base font-bold active:scale-[0.96] transition ${farben.buttonSek}`}
            >
              Löschen
            </button>
          </div>
        ...
        {!feedback ? (
          <button onClick={pruefen} className="w-full mt-4 bg-green-600 text-white rounded-xl p-3 text-base font-semibold active:scale-[0.98] transition">
            Prüfen
          </button>
        ) : (
          <>
            <div className={`mt-5 rounded-2xl p-4 text-center ${feedback.richtig ? "bg-green-900/30" : "bg-red-900/30"}`}>
              <p className={`font-bold text-2xl ${feedback.richtig ? "text-green-400" : "text-red-400"}`}>
                {feedback.richtig ? "Richtig Richtig!" : "Falsch Falsch"}
              </p>
            ...
            <button onClick={weiter} className="w-full mt-4 bg-green-600 text-white rounded-xl p-3 text-base font-semibold active:scale-[0.98] transition">
  ...
  return (
    <Rahmen>
      ...
      {falsch > 0 && (
        <button onClick={() => quizStarten(true)} className="w-full bg-orange-500 text-white rounded-xl p-3 text-base font-semibold mb-3 active:scale-[0.98] transition">
          Fehler erneut üben
        </button>
      )}

      <button onClick={() => setAnsicht("start")} className="w-full bg-green-600 text-white rounded-xl p-3 text-base font-semibold active:scale-[0.98] transition">
        Neues Quiz starten
      </button>
